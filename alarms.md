...
10. Clicking an alarm card opens its edit dialog. Clicking the enabled checkbox or Dismiss control must not open the dialog.
11. The enabled checkbox on the card is the manual way to enable or disable an alarm without editing its time or schedule.
12. A Dismiss control is shown above the enabled checkbox only when an enabled alarm is due within the next two hours. The view refreshes this eligibility as time passes, and a dismissed upcoming occurrence is visibly marked as dismissed.
...
6. Verify Dismiss appears as an alarm reaches two hours away, marks the selected upcoming occurrence as dismissed, stops active audio, and leaves later occurrences enabled.
